...
const TopCategories = () => {
  ...
  return (
    <div className="p-3 sm:p-4 md:p-6 bg-gray-100 rounded-md shadow-md sm:shadow-lg md:shadow-2xl shadow-gray-200">
      <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-2 sm:mb-4">Top Categories</h2>
      <div className="relative overflow-hidden">
        <Slider {...settings}>
          {categories.map((cat, index) => (
            <div key={index} className="text-center px-1 sm:px-2 md:px-3">
              <div className="w-16 h-16 sm:w-24 sm:h-24 md:w-32 md:h-32 lg:w-36 lg:h-36 mx-auto 
              ...
              <p className="mt-1 sm:mt-2 text-xs sm:text-sm truncate max-w-full">{cat.name}</p>
            </div>
          ))}
        </Slider>
      </div>
